...
void solve(){
    ...
    f(n){
        curr = (i > 0 ? abs(v[i] - v[i - 1]) : 0) + (i < n - 1 ? abs(v[i] - v[i + 1]) : 0);
        
        // for 1 :
        New = (i > 0 ? abs(1 - v[i - 1]) : 0) + (i < n - 1 ? abs(1 - v[i + 1]) : 0);
        if(New > curr) mx = max(mx, sum + (New - curr) );
          
        // for k :
        New = (i > 0 ? abs(k - v[i - 1]) : 0) + (i < n - 1 ? abs(k - v[i + 1]) : 0);
        if(New > curr) mx = max(mx, sum + (New - curr) );
    }
