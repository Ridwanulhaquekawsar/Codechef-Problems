...
void solve(){
    ...
    c_01=0,c_10=0;
    for(int i=0; i<n-1; i++){
        if(s[i]=='0' && s[i+1]=='1') c_01++;
        else if(s[i]=='1' && s[i+1]=='0') c_10++;
    }
    
    if(c_01>0 && c_10>0) cout<<"Alice"<<'\n';
    else cout<<"Bob"<<'\n';
}
...
void solve(){
    ...
    f(n - 1) if(s[i] != s[i + 1]) transition++;
    
    (transition > 1) ? out("Alice") : out("Bob");
}
